...
rating, comment) => __awaiter(void 0, void 0, void 0, function* () {
    return yield database_service_1.default.transaction((t) => __awaiter(void 0, void 0, void 0, function* () {
        const ride = yield ride_model_1.default.findByPk(rideId, { transaction: t });
        if (!ride)
        ...
        // Logic check: You can only rate completed rides.
        if (ride.status !== ride_model_1.RideStatus.COMPLETED) {
        ...
        const existingRating = yield rating_model_1.default.findOne({ where: { rideId, raterId, ratedId }, transaction: t });
        if (existingRating) {
        ...
        // Update the average rating of the user who was rated using SQL aggregation
        const result = yield rating_model_1.default.findOne({
            where: { ratedId },
            attributes: [[database_service_1.default.fn('AVG', database_service_1.default.col('rating')), 'avgRating']],
            transaction: t
        });
        const averageRating = result ? parseFloat(result.getDataValue('avgRating')) : rating;
        if (ratedUserType === 'driver') {
            yield driver_model_1.default.update({ averageRating: averageRating }, { where: { id: ratedId }, transaction: t });
        }
        else {
            yield user_model_1.default.update({ averageRating: averageRating }, { where: { id: ratedId }, transaction: t });
        }
        return newRating;
    }));
});
